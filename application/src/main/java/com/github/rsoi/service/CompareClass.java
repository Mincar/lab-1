...
public class CompareClass {
    ...
    public void comparePhones(ArrayList<phone> phoneArrayList)
    { int counter=0;
        for(phone phoneO: phoneArrayList)
        {
        this.compare(phoneO,ram, size, sd);
        }
        for(phone phoneO: phoneArrayList)
        {
            this.compare(phoneO,ram, size, sd);
        ...
        if(counter==0){System.out.println("По вашему запросу ничего не найдено"); return;}
